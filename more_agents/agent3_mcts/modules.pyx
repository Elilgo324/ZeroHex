...
cdef class node:
  ...
  cpdef float value(self, float explore, int rave_const, int eval = 0):
    ...
    cdef dict eval_methods = {'uct':0, 'rave':1, 'ucb1-tuned':2, 'rave-ucb1-tuned':3, 'amaf':4}
    ...
    if (self.N == 0):
      ...
      if eval == eval_methods['uct']:
        ...
        if self.N_RAVE != 0:
          AMAF = self.Q_RAVE / self.N_RAVE
        else:
          AMAF = 0
        return (1-alpha) * UCT + alpha * AMAF

      elif eval == eval_methods['ucb1-tuned']:
        variance = EXPLOIT * (1 - EXPLOIT)
        return EXPLOIT + explore * sqrt(log(self.parent.N) / self.N) * min(0.25, variance + EXPLORE)

      elif eval == eval_methods['amaf']:
        return self.Q_RAVE/ self.N_RAVE + sqrt(2 * log(self.parent.N_RAVE) / self.N_RAVE)
...
cdef class ucb1_tuned_agent(mctsagent):
  """
  Basic no frills implementation of an agent that preforms MCTS for hex.
  ...
  cpdef tuple select_node(self):
    ...
    while(len(new_node.children) !=0 ):
      #descend to the maximum value node, break ties at random
      length = len(new_node.children)
      max_value = new_node.children[0].value(self.EXPLORATION, self.RAVE_CONSTANT, 2)
      for i in range(1,length):
        if new_node.children[i].value(self.EXPLORATION, self.RAVE_CONSTANT, 2) > max_value:
          max_value = new_node.children[i].value(self.EXPLORATION, self.RAVE_CONSTANT, 2)
      
      max_nodes = [n for n in new_node.children 
                   if n.value(self.EXPLORATION, self.RAVE_CONSTANT, 2) == max_value]
      new_node = random.choice(max_nodes)
      state.play(new_node.move)
...
#############################                                                    ######################################
#############################                                                    ######################################
#############################         QUALITY-BASED REWARDS ALGORITHMS           ######################################
#############################                                                    ######################################
#############################                                                    ######################################
#######################################################################################################################
#######################################################################################################################
...
class qb_mctsagent_py(mctsagent):
  """
  Basic no frills implementation of an agent that preforms MCTS for hex.
  ...
  def __init__(self, state=gamestate(8)):
    ...
    self.a_const, self.k_const = 0.25, 1
    self.pl_list = np.asarray([[0, 0]])


  def search(self, int time_budget):
    """
    Search and update the search tree for a 
    specified amount of time in secounds.
  ...
  def roll_out(self, gamestate state):
    ...
    """
    moves = state.moves().copy()  # Get a list of all possible moves in current state of the game
  ...
  def backup(self, node node, int turn, int outcome, list pl_length=None):
    """
    Update the node statistics on the path from the passed node to root to reflect
    the outcome of a randomly simulated playout.
    ...
    cdef dict bonus = self.modify_reward(pl_length)
    cdef int reward
    reward = -1 if outcome == turn else 1

    while node != None:
      node.N += 1
      if turn == self.rootstate.PLAYERS['black']:
        q_reward = reward + (reward * self.a_const * bonus['black'])
      else:
        q_reward = reward + (reward * self.a_const * bonus['white'])
      node.Q += q_reward
...
class qb_ucb1_tuned_agent_py(qb_mctsagent_py):
  """
  Basic no frills implementation of an agent that preforms MCTS for hex.
...
class Gui:
  ...
  def __init__(self, root, agent_name='UCT'):
    ...
    self.AGENTS = {  
              ...
              "RAVE": rave_mctsagent,
              "QBR-UCT": qb_mctsagent_py,
              "QBR-UCB1-TUNED": qb_ucb1_tuned_agent_py,
              "LGR":lgr,
